...
Google previously released [jaxopt](https://github.com/google/jaxopt/) to provide this functionality but this project is now deprecated with the code being merged into DeepMind's [Optax](https://github.com/google-deepmind/optax).
...
::: {.xsmall}
```{python}
optimizer = optax.adam(learning_rate=2.5, b1=0.9, b2=0.999, eps=1e-8)
...
::: {.xsmall}
```{python}
#| code-line-numbers: "|5,6|8-10,13|14-17|19-22"
...
* For really large models batches are a necessity and sizing is often determined by memory constraints
...
As mentioned last time, these gradient based are not guaranteed to converge unless their learning rates decay as a function of step.
...
adam_runtime = {
  batch_size: timeit.Timer( lambda:
    optax_optimize(
      params=jnp.zeros(k), X=X, y=y, loss_fn=lr_loss, 
      optimizer=optax.adam(
        #learning_rate=optax.schedules.linear_schedule(1, 0.05, 2*n/batch_size), 
        learning_rate=optax.schedules.exponential_decay(
          init_value=1,
          transition_steps=100, 
          decay_rate=0.9
...
The only missing method from `scipy` is `Newton-CG` and there is the addition of the `SANN` method which is a variant of simulated annealing and does not require gradient information. However, it is slow and very sensitive to its control parameters and is not generally considered a general-purpose method.
...
::: {.column}
```{r}
optim(x0, f, grad, method = c("BFGS"))
```
:::
::: {.column}
```{r}
optim(x0, f, grad, method = c("CG"))
```
:::
:::
...
For any of these algorithms you will generally be depending on the underlying library to make them available to you, for example:
...
* Provides much better support for constrained optimization problems
...
nloptr::nloptr(
  ...
  opts = list(
    "algorithm" = "NLOPT_LD_LBFGS", "xtol_rel" = 1.0e-8
  )
)
```
:::
...
![](imgs/NLopt-example-constraints.webp){width="50%" fig-align=center}
...
f = function(x, a, b) sqrt(x[2])
grad_f = function(x, a, b)  c(0, 0.5 / sqrt(x[2]))

# Constraint function
g = function(x, a, b) (a * x[1] + b) ^ 3 - x[2]
